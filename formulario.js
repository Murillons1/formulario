
let res = document.querySelector("#res")
let res2 = document.querySelector("#res2")
let btn_enviar = document.querySelector("#btn_enviar")
let btn_calcular = document.querySelector("#btn_calcular")
let prod_array = []
let qtde_array = []
let preco_laranja = 5.49, preco_limao = 1.49, preco_pera = 2.49


btn_enviar.addEventListener("click", ()=>{
   res.innerHTML = " " // Para atualizar e escrever na tela o ultimo elemento selecionado
   let produto  = document.querySelector("#produto").value
   let qtde  = document.querySelector("#qtde").value
   prod_array.push(produto)
   qtde_array.push(qtde)
   console.log(qtde_array)
   console.log(prod_array)
   res.innerHTML = "</br>" + produto + "  e a quantidade = " + qtde +  " kg"

})


btn_calcular.addEventListener("click",()=>{
    let total= 0.0
    for(i=0;i<prod_array.length;i++){
        if(prod_array[i]== "laranja"){
            total = total + preco_laranja*qtde_array[i]
        }
        if(prod_array[i]== "limao"){
            total = total + preco_limao*qtde_array[i]
        }
        if(prod_array[i]== "pera"){
            total = total + preco_pera*qtde_array[i]
        }

    }
    console.log(total)
    res2.innerHTML = "</br>"  + "  O valor total é = " + total.toFixed(2)
    res2.style.color = "white"
    res2.style.fontweigh = "bolder"
    res2.style.fontSize = "1.4rem"

})


